...
4. `skew()`：按指定角度倾斜元素，可以分别指定水平和垂直方向的倾斜角度。
...
### 总结： HTML+CSS 占分比重  5 - 20 分。高频考点：`flex 布局`,`grid 布局`,`css3 新特性` 

### 备赛建议：HTML + CSS 题目较为简单，题目数量 1 - 2，耗时较短，分数一定要全部拿到。
...
### 总结： 函数封装占分比重  30 - 70 分。题目数量：2 - 4。高频考点：`递归函数` 、`回调函数`、`数组方法`、`字符串方法`、`随机数`
